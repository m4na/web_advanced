...
public class Web_advancedApplication extends Application {
	...
	public void init() {
		...
		mysql oracle = new mysql("jdbc:mysql://localhost:3306/web_advanced", "root", "root");
		
		// mapping with the table VAADIN_EMPLOYEE of the Oracle database
		phonebook = oracle.queryTable("test");
		...
		hl.setSpacing(true);
		hl.setMargin(true);
		Button addButton = new Button("Add", phonebook, "addItem"); // adds an
																	// empty row
																	// into the
																	// container
																	// and thus
	...
	// delete a record
	public void delete() {
		RowId itemID = (RowId) table.getValue(); // retrieves the id of the
													// record
		if (itemID != null) {
			phonebook.removeItem(itemID); // remove the record from the
											// container
